...
void teaser::TLSScaleSolver::distributionInlierSelection(const Eigen::Matrix<double, 3, Eigen::Dynamic> &src,
                                                         ...
                                                         const std::vector <Eigen::Matrix3d> &tgt_covs,
                                                         double *scale,
                                                         Eigen::Matrix<bool, 1, Eigen::Dynamic> *inliers) {}
...
void teaser::ScaleInliersSelector::distributionInlierSelection(const Eigen::Matrix<double, 3, Eigen::Dynamic> &src,
    ...
    *scale = 1;
    ...
    // waterstein distance helps a little bit or not
...
#pragma omp parallel for default(none) shared(N, norm_inliers, src_covs, tgt_covs, temp_wasserstein, noise_distribution)
    for (int i = 0; i < N; ++i) {
        if (norm_inliers(i)) {
            temp_wasserstein = (src_covs[i] + tgt_covs[i] -
                                2 * (src_covs[i].sqrt() * tgt_covs[i] * src_covs[i].sqrt()).sqrt()).trace();
            //LOG(INFO) << "temp_wasserstein: " << temp_wasserstein << std::endl;
...
teaser::RobustRegistrationSolver::computeTIMsWithCov(const Eigen::Matrix<double, 3, Eigen::Dynamic> &v,
                                                     ...
                                                     std::vector <Eigen::Matrix3d> &covtiled,
...
// #pragma omp parallel for shared(N, v, vtilde, map, cov, covtiled)
    for (size_t i = 0; i < N - 1; i++) {
        ...
        // k*N - k*(k+1)/2
        size_t segment_start_idx = i * N - i * (i + 1) / 2; // starting index of the segment
        size_t segment_cols = N - 1 - i; // number of columns in the segment

        // calculate TIM
        Eigen::Matrix<double, 3, 1> m = v.col(i);
        // 对于第i行，取出第i行的所有列，然后减去第i列的值，构造平移不变量
        ...
        std::vector <Eigen::Matrix3d> temp_cov = cov;
...
teaser::RobustRegistrationSolver::solve(const Eigen::Matrix<double, 3, Eigen::Dynamic> &src,
    ...
    std::vector <Eigen::Matrix3d> src_covtiled;
    std::vector <Eigen::Matrix3d> tgt_covtiled;
    // 如果匹配个数是N，那么src_tims_和dst_tims_的大小是N*(N-1)/2
...
void teaser::RobustRegistrationSolver::solveForScaleWithCov(const Eigen::Matrix<double, 3, Eigen::Dynamic> &v1,
                                                            const Eigen::Matrix<double, 3, Eigen::Dynamic> &v2,
                                                            const std::vector <Eigen::Matrix3d> &src_covtiled,
                                                            const std::vector <Eigen::Matrix3d> &tgt_covtiled) {
    scale_inliers_mask_.resize(1, v1.cols());
    scale_solver_->distributionInlierSelection(v1, v2, src_covtiled, tgt_covtiled, &(solution_.scale),
                                               &scale_inliers_mask_);
    // scale_solver_->solveForScale(v1, v2, &(solution_.scale), &scale_inliers_mask_);
//    std::cout << "Scale: " << solution_.scale << std::endl;
}
