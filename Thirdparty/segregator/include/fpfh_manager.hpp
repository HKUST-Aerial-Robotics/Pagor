...
public:
    ...
    FPFHManager(double normal_radius, double fpfh_radius, int interval = 1) : normal_radius_(normal_radius),
    ...
    void setFeaturePair(pcl::PointCloud<PointType>::Ptr src, pcl::PointCloud<PointType>::Ptr target) {
        ...
        for (size_t i = 0; i < corr.size(); ++i) {
            ...
            tgt_normals.col(i) << double(tgt_normals_raw->points[dst_idx].normal_x), double(
                    tgt_normals_raw->points[dst_idx].normal_y), double(tgt_normals_raw->points[dst_idx].normal_z);
        }
